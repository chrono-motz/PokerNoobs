'''
Bot v3 — Tight-Shark
Very selective preflop, massive value bets when strong, never bluffs.
Pure value extracting machine.
'''
...
class Player(BaseBot):
    ...
    # ── TIGHT PREFLOP: only play top hands ──
    def _preflop(self, s):
        chen = self._chen
        cost = s.cost_to_call
        ...
        # Everything else: fold (very tight)
    ...
    def _bid(self, s):
        ...
        pot = s.pot
        chips = s.my_chips
        # Tight-Shark bids conservatively — save chips for actual bets
        uncertainty = 1.0 - abs(eq - 0.5) * 2
        bid_value = pot * 0.12 * uncertainty
        bid_value = min(bid_value, chips * 0.06)
        bid = max(0, min(int(bid_value), chips))
        return ActionBid(bid)

    # ── SHARK POSTFLOP: very high thresholds, massive value bets, NEVER bluff ──
    def _postflop(self, s, game_info):
        ...
        # Decent — only bet if very clearly ahead
        if eq >= 0.55:
            if cost == 0:
                if s.can_act(ActionRaise):
                    lo, hi = s.raise_bounds
                    amt = min(hi, max(lo, int(pot * 0.50)))
                    return ActionRaise(amt)
                return ActionCheck()
            if eq > pot_odds + 0.08:
                return ActionCall() if s.can_act(ActionCall) else ActionCheck()
        ...
        # Marginal — very cautious
        if eq >= 0.40:
            if cost == 0: return ActionCheck()
            if eq > pot_odds + 0.05 and cost <= pot * 0.25:
                return ActionCall() if s.can_act(ActionCall) else ActionCheck()
            return ActionCheck() if s.can_act(ActionCheck) else ActionFold()

        # Anything weaker — just fold (NO bluffs)
        return ActionCheck() if s.can_act(ActionCheck) else ActionFold()
    ...
    def _bet_value(self, s, aggressive):
        ...
        if cost > 0:
            if aggressive and s.can_act(ActionRaise):
                lo, hi = s.raise_bounds
                # Overbet for max value
                amt = min(hi, max(lo, int(pot * 0.90) + s.opp_wager))
                return ActionRaise(amt)
            return ActionCall() if s.can_act(ActionCall) else ActionCheck()
        if s.can_act(ActionRaise):
            lo, hi = s.raise_bounds
            frac = 0.90 if aggressive else 0.65
