'''
Bot v4 — Auction-Hunter
Bids aggressively to win auction info, then exploits the revealed card heavily.
'''
...
from bot import mc_equity, chen_score, RANK_MAP, MC_ITERS_AUCTION, MC_ITERS_POSTFLOP
...
class Player(BaseBot):
    ...
    def _track(self, s):
        ...
        if len(s.opp_revealed_cards) > 0: self._has_info = True
    ...
    def _bid(self, s):
        ...
        bid_value = pot * 0.35 * uncertainty
        if eq > 0.70: bid_value = max(bid_value, pot * 0.15)
        bid_value = min(bid_value, chips * 0.18)
    ...
    def _postflop(self, s, gi):
        ...
        m = 1.25 if self._has_info else 1.0

        if eq >= 0.80: return self._vbet(s, True, m)
        if eq >= 0.63: return self._vbet(s, False, m)
        if eq >= 0.50:
            if cost == 0:
                if s.can_act(ActionRaise):
                    lo, hi = s.raise_bounds
        ...
        if eq >= 0.35:
            ...
            return ActionCheck() if s.can_act(ActionCheck) else ActionFold()
        if eq >= 0.20 and self._has_info and cost == 0:
