...
class Player(BaseBot):
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        try:
            ...
            # If opponent tracking fails, continue anyway
            pass
        ...
        try:
            ...
            # Fallback: if anything crashes, return minimal valid action
            # IMPORTANT: Always return a legal action or it gets interpreted as a fold
            if street == 'auction':
                return ActionBid(1)
            # For pre-flop or postflop, call or check or fold (always safe)
    ...
    def _preflop(self, state):
        """Preflop: VERY WIDE AND AGGRESSIVE."""
        try:
            chen = self._chen
            cost = state.cost_to_call
            
            # Simple strategy: with any decent hand, always play
            # Premium: raise
            if chen >= 7:
            ...
            if state.can_act(ActionCheck):
                return ActionCheck()
            elif state.can_act(ActionCall) and cost <= BIG_BLIND:
                return ActionCall()
            else:
                return ActionFold()
        
        except Exception as e:
            # If preflop crashes, just call or fold
            if state.can_act(ActionCall):
                return ActionCall()
            elif state.can_act(ActionCheck):
                return ActionCheck()
    ...
    def _auction(self, state):
        """Smart auction bidding: bid based on equity strength - DEFENSIVE VERSION."""
        try:
        ...
        try:
            ...
            # DEFENSIVE: Ensure pot and chips are positive integers
            if pot <= 0:
                pot = 30  # Default minimum pot value
            if chips <= 0:
                chips = 4990  # Default chip value
            ...
            # Calculate bid - these are ALL guaranteed >= 1
            base_bid = int(pot * bid_pct)
            capped_bid = min(base_bid, int(chips * 0.15))
            
            # ABSOLUTE GUARANTEE: bid is at least 1
            final_bid = max(1, capped_bid, base_bid)
            
            # Final safeguard before return
            if final_bid < 1:
                final_bid = 1
            ...
            # If ANYTHING goes wrong, bid 1 chip as fallback
            return ActionBid(1)
