#!/usr/bin/env python3
"""
Poker Bot V6 - Safe & Balanced Edition
Fixes critical bugs from advanced bot:
1. SAFE auction bidding (never illegal amounts)
2. Position-aware hand strength
3. Stack-aware tightening (short stacks play tighter)
4. Pot odds based postflop decisions
5. Reduced bluff calling from 50% to 25%
"""
...
class Player(BaseBot):
    """Safe poker bot with proper auction bidding and position awareness"""
    ...
    def _preflop(self, state):
        """Position-aware preflop - tighter on short stacks"""
        chen = self._chen
        cost = state.cost_to_call
        my_chips = state.my_chips
        
        # Stack adjustment: short stacks play tighter
        stack_ratio = my_chips / STARTING_STACK
        if stack_ratio < 0.5:
            chen -= 2  # Tight
        ...
        if cost == 0:
            # Check/raise position
            if chen >= 8:
                if state.can_act(ActionRaise):
                    lo, hi = state.raise_bounds
                    amt = min(hi, max(lo, int(2.5 * BIG_BLIND) + state.opp_wager))
                    return ActionRaise(amt)
                return ActionCall() if state.can_act(ActionCall) else ActionCheck()
            elif chen >= 5:
                if state.can_act(ActionRaise) and random.random() < 0.5:
                    lo, hi = state.raise_bounds
                    return ActionRaise(min(hi, max(lo, int(2 * BIG_BLIND) + state.opp_wager)))
            ...
            if chen >= 9:
                ...
                return ActionCall() if state.can_act(ActionCall) else ActionCheck()
            elif chen >= 3 and cost <= 10:
                return ActionCall() if state.can_act(ActionCall) else ActionCheck()
            else:
                return ActionCheck() if state.can_act(ActionCheck) else ActionFold()

    def _auction(self, state):
        """SAFE auction - never exceed 20% of chips"""
        try:
            equity = mc_equity(state.my_hand, list(state.board) if state.board else [], [], 25)
        except:
            equity = 0.5
        ...
        if pot <= 0: pot = 30
        if chips <= 0: chips = STARTING_STACK
        
        # CRITICAL SAFETY: Max bid is 20% of remaining chips
        max_bid = max(1, int(chips * 0.20))
        
        # Equity-based scaling  
        if equity >= 0.70:
            bid = min(int(pot * 0.15), max_bid)
        elif equity >= 0.60:
            bid = min(int(pot * 0.10), max_bid)
        elif equity >= 0.50:
            bid = min(int(pot * 0.06), max_bid)
        elif equity >= 0.40:
            bid = min(int(pot * 0.03), max_bid)
        else:
            bid = max(1, min(int(pot * 0.01), max_bid))
        
        return ActionBid(max(1, min(bid, chips)))

    def _postflop(self, state, game_info):
        """Equity-based with pot odds"""
        if game_info.time_bank < 1.5:
        ...
        rollouts = 25 if state.street == 'flop' else 35
        try:
        ...
        pot = state.pot
        cost = state.cost_to_call
        po = cost / (pot + cost + 1) if cost > 0 else 0
        ...
        if equity >= 0.50:
            if cost == 0:
                if state.can_act(ActionRaise) and random.random() < 0.3:
                    lo, hi = state.raise_bounds
                    bet = int(pot * 0.30)
                    return ActionRaise(min(hi, max(lo, bet)))
                return ActionCheck()
        ...
        if equity >= 0.40:
            if cost == 0:
                return ActionCheck()
            if equity > po and cost <= pot * 0.25:
                return ActionCall() if state.can_act(ActionCall) else ActionCheck()
            return ActionCheck() if state.can_act(ActionCheck) else ActionFold()
