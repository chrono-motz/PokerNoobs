'''Bot v2 — Aggressive: wider ranges, bigger bets, more bluffs.'''
...
class Player(BaseBot):
    ...
    def _preflop(self, s):
        ...
        if c >= 3:
            if not s.is_bb and cost <= BIG_BLIND and s.can_act(ActionRaise) and random.random() < 0.35:
                lo, hi = s.raise_bounds
    ...
    def _bid(self, s):
        ...
        u = 1.0 - abs(eq - 0.5) * 2
        bv = min(s.pot * 0.18 * u, s.my_chips * 0.10)
    ...
    def _postflop(self, s, gi):
        ...
        if eq >= 0.78: return self._vb(s, True)
        if eq >= 0.60: return self._vb(s, False)
        if eq >= 0.45:  # lower than baseline (0.50)
            if cost == 0:
                if s.can_act(ActionRaise):
                    lo, hi = s.raise_bounds
                    return ActionRaise(min(hi, max(lo, int(pot*0.55))))
                return ActionCheck()
            if eq > po + 0.03: return ActionCall() if s.can_act(ActionCall) else ActionCheck()
            return ActionCheck() if s.can_act(ActionCheck) else ActionFold()
        if eq >= 0.30:
            if cost == 0:
                if self.fold_rate > 0.3 and random.random() < 0.20:  # more bluffs
                    if s.can_act(ActionRaise):
                        lo, hi = s.raise_bounds
                        return ActionRaise(min(hi, max(lo, int(pot*0.60))))
                return ActionCheck()
            if eq > po and cost <= pot*0.4: return ActionCall() if s.can_act(ActionCall) else ActionCheck()
            return ActionCheck() if s.can_act(ActionCheck) else ActionFold()
        if eq >= 0.15 and cost == 0:
    ...
    def _vb(self, s, agg):
        ...
        if s.can_act(ActionRaise):
            lo, hi = s.raise_bounds
            return ActionRaise(min(hi, max(lo, int(pot*(0.85 if agg else 0.60)))))
        return ActionCheck()
